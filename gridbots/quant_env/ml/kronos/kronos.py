...
def auto_regressive_inference(tokenizer, model, x, x_stamp, y_stamp, max_context, pred_len, clip=5, T=1.0, top_k=0, top_p=0.99, sample_count=5, verbose=False, return_raw=False):
    with torch.no_grad():
        ...
        raw_np = z.cpu().numpy()  # (batch_size, sample_count, pred_len, n_feat)

        if return_raw:
            preds = np.mean(raw_np, axis=1)  # (batch_size, pred_len, n_feat)
            raw_samples = raw_np  # (batch_size, sample_count, pred_len, n_feat)
            return preds, raw_samples

        preds = np.mean(raw_np, axis=1)
        return preds


def calc_time_stamps(x_timestamp):
    time_df = pd.DataFrame()
    # Works for both DatetimeIndex and Series with datetime dtype
