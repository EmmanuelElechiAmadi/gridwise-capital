...
class BacktestEngine:
    ...
    def run(self):
        ...
        for idx, bar in self.data.iterrows():
            ...
            # ── 3. Notify strategy of the new bar ──
            strat.on_bar({
                'open': float(bar['open']),
                'high': high,
                'low': low,
...
class MockConnector:
    """Mock connector used by BacktestEngine to talk to a strategy."""

    def __init__(self, engine):
        self.engine = engine
        # Build config from engine's strategy_kwargs with uppercase names
        cfg = {
            'SYMBOL': 'BACKTEST',
            'LOT_SIZE': float(engine.strategy_kwargs.get('lot', 0.01)),
            'GRID_SPACING': float(engine.strategy_kwargs.get('spacing', 0.1)),
            'NUM_LEVELS': int(engine.strategy_kwargs.get('levels', 5)),
            'LOOKBACK_4H_BARS': int(engine.strategy_kwargs.get('lookback_4h_bars', 5)),
            'BREAKOUT_THRESHOLD_PCT': float(
                engine.strategy_kwargs.get('breakout_threshold_pct', 0.05)),
            'CONFIRMATION_BARS_1H': int(
                engine.strategy_kwargs.get('confirmation_bars_1h', 2)),
            'MAX_POSITIONS': int(engine.strategy_kwargs.get('max_positions', 1)),
            'TP_DOLLARS': str(engine.strategy_kwargs.get('tp_dollars', '3,5,10')),
            'SL_DOLLARS': float(engine.strategy_kwargs.get('sl_dollars', 3.0)),
        }
        self.config = type('obj', (), cfg)
    ...
    def close_all_positions(self):
        """Mock close-out — liquidate inventory at the current bar price."""
        self.engine._liquidate_inventory(self.engine.data.iloc[min(
            self.engine._bar_index, len(self.engine.data) - 1)])
